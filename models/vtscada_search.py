...
Examples
--------
# 1. Run grid search & freeze best (English docs)
python vtscada_search.py --mode grid \
       --docs vtscada_functions_en.json \
       --queries eval_queries_en.json \
       --lang en \
       --workers -1 \
       --out grid_results_en.csv

# -> creates grid_results.pkl in same folder

# 2. Grid search & freeze (Chinese docs)
python vtscada_search.py --mode grid \
  --docs vtscada_functions_zh-TW.json \
  --queries eval_queries_zh-TW.json \
  --lang zh-TW \
  --workers -1 \
  --out grid_results_zh-TW.csv

# → writes grid_results_zh-TW.pkl

# 3. Ultra-fast inference using frozen model
python vtscada_search.py --mode infer \
       --model grid_results_en.pkl \
       --docs vtscada_functions_en.json \
       --lang en \
       --query "acknowledge alarm" --topk 5

# 4. Inference (frozen, offline, CPU) (Chinese query)
python vtscada_search.py --mode infer \
  --model grid_results_zh-TW.pkl \
  --docs vtscada_functions_zh-TW.json \
  --lang zh-TW \
  --query "確認警報" --topk 5
"""
...
import unicodedata
try:
    import jieba  # optional, better segmentation for Chinese
    HAS_JIEBA = True
except Exception:
    HAS_JIEBA = False
# ------------------------------ Data Model ------------------------------
...
class VTScadaSearcher:
    ...
    def _make_vectorizer(self):
        lang = self.cfg.get("language", "en").lower()
        ...
        if self.cfg["vectorizer_type"] == "tfidf":
            ...
            if lang == "auto":
                return TfidfVectorizer(tokenizer=lambda s: tokenize(s, "auto"),
                                    preprocessor=None, lowercase=False,
                                    token_pattern=None, **common)
            # English
            if self.cfg["use_stemming"]:
    ...
    def query(self, q: str, top_k=10) -> List[Tuple[int, float]]:
        if self.cfg["vectorizer_type"] == "bm25":
            ...
            sims = self.bm25.get_scores(q_tok)
...
def _normalize(text: str) -> str:
    # Normalize full/half-width characters; do not lower here (English lower is done later)
    return unicodedata.normalize("NFKC", text)

def zh_tokens(text: str) -> List[str]:
    """
    Prefer jieba if available; otherwise fall back to character bigrams.
    """
    txt = _normalize(text)
    if HAS_JIEBA:
        toks = [t.strip() for t in jieba.lcut(txt, cut_all=False) if t.strip()]
        return toks
    # fallback: char bigrams (robust, no deps)
    s = re.sub(r"\s+", "", txt)
    if len(s) <= 1:
        return list(s)
...
def en_tokens(text: str) -> List[str]:
    """
    Extract English-ish words/numbers (incl. underscores/dashes), lowercased.
    """
    txt = _normalize(text)
    words = re.findall(r"[A-Za-z0-9]+(?:[-_][A-Za-z0-9]+)*", txt)
    return [w.lower() for w in words if w]

def tokenize(text: str, language: str = "en") -> List[str]:
    """
    Unified entry-point. For mixed content, use language='auto'.
    """
    if language.lower() in ("zh", "zh-tw", "zh-cn"):
        return zh_tokens(text)
    if language.lower() == "auto":
        ...
        return z + e
    # default English
    clean = " ".join(re.sub(r"[^\w\s]", " ", text.lower()).split())
    return clean.split()
...
def run_gridsearch(
    ...
    # If Chinese, skip stemming and prefer char bigrams/trigrams
    if language.lower() in ("zh", "zh-tw", "zh-cn", "auto"):
        vectorizer_types = ["count", "tfidf", "bm25", "dense"]
        use_stemmings = [True, False]
        min_dfs = [1, 3, 5, 10]
        max_dfs = [0.5, 0.8, 1.0]
        ...
        name_weights = [1, 3, 5]
        clusters_list = [None, 20, 50, 100]
        # clusters_list = [None, 20, 50]
        # character n-grams work better as (2,3) for zh

    if language.lower() in ("zh", "zh-tw", "zh-cn", "auto") and dense_model == "sentence-transformers/all-MiniLM-L6-v2":
    ...
    def valid(cfg):
        ...
        if vtype == "bm25":
            return (stem and min_df == 1 and max_df == 1.0 and (ngr == (1, 1) or ngr == (2, 3)) and n_clusters is None)
        if vtype == "dense":
            return (stem and min_df == 1 and max_df == 1.0 and (ngr == (1, 1) or ngr == (2, 3)) and n_clusters is None)
        return True
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--lang", default="en",
        choices=["en", "zh", "zh-TW", "zh-CN", "auto"],
        help="Language of the docs & queries. Use 'auto' for mixed English + Chinese."
    )
