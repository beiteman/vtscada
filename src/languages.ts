...
// const SIMPLIFIED_ONLY = new Set(['国','车','马','门','风','云','鱼','鸟','书','买','卖','东','西','广','乐','万','与','专','业','丢','严','丽','举','义','乌']);
// const TRADITIONAL_ONLY = new Set(['國','車','馬','門','風','雲','魚','鳥','書','買','賣','東','西','廣','樂','萬','與','專','業','嚴','麗','舉','義','烏']);
...
const SIMPLIFIED_ONLY = new Set([...S]);
const TRADITIONAL_ONLY = new Set([...T]);
...
export class LanguageIdentifier {
    public identify(text: string): Lang {
        let latinCount = 0
        let chineseCount = 0
        ...
        for (const ch of text) {
            if (isLatin(ch)) {
                ...
                if (SIMPLIFIED_ONLY.has(ch)) simplifiedScore++
                if (TRADITIONAL_ONLY.has(ch)) traditionalScore++
            }
        }
        ...
        if (chineseCount > 0 && latinCount === 0) {
            if (simplifiedScore > traditionalScore) return 'zh-cn'
            if (traditionalScore > simplifiedScore) return 'zh-tw'
            return 'zh-cn' // shared chars only
        }
